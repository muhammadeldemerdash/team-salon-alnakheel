...
function applySite(html) {
  const replacements = [
    ...
    [/\+966 55 470 2651/g, site.phoneDisplay],
    [/\+966554702651/g, site.phone],
    [/966554702651/g, site.whatsapp],
    [/حي النخيل، الرياض 12611، المملكة العربية السعودية/g, site.addressFull],
    [/حي النخيل، الرياض 12611/g, site.addressShort],
    [/فرع النخيل/g, `فرع ${site.branchName}`],
    [/Team Salon النخيل/g, site.siteName],
...
for (const entry of fs.readdirSync(templates, { withFileTypes: true })) {
  ...
  if (entry.name === 'index.html') {
    ...
    html = replaceMarked(html, 'ARTICLES', articleCards('articles/'));
...
for (const article of articles) {
  ...
  html = html.replace(/<link rel="canonical" href="[^"]*">/, `<link rel="canonical" href="${site.domain.replace(/\/$/, '')}/articles/${esc(a.slug)}.html">`);
  const schema = {
...
const urls = [
  ...
  ...staticPages.filter(p => fs.existsSync(path.join(dist, p))).map(p => ({ loc: `${base}/${p}`, freq: p === 'offers.html' ? 'weekly' : 'monthly', priority: p === 'offers.html' ? '0.9' : '0.7' })),
  { loc: `${base}/articles/`, freq: 'weekly', priority: '0.8' },
  ...articles.map(a => ({ loc: `${base}/articles/${a.data.slug}.html`, freq: 'monthly', priority: '0.7' }))
];
...
console.log(`Built ${staticPages.length + articles.length + 2} pages in dist/`);
