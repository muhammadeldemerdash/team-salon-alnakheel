...
for (const article of articles) {
  ...
  if (a.image) {
    ...
    schema.image = image.startsWith('http') ? image : `${site.domain.replace(/\/$/, '')}${image}`;
  }
  ...
  const articleImage = a.image
    ? `<img class="article-featured-image" src="${esc(assetUrl(a.image))}" alt="${esc(a.title)}">`
    : '';
